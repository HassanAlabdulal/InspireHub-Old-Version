...
import { faStar as solidStar, faStarHalfAlt, faArrowRight } from '@fortawesome/free-solid-svg-icons';
    ...
    const projects = [
      ...
      const renderStars = (rate) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= rate) {
                // Full star
                stars.push(<FontAwesomeIcon key={`full_${i}`} icon={solidStar} className="text-yellow-500" />);
            } else if (i === Math.ceil(rate) && rate % 1 !== 0) {
                // Half star
                stars.push(<FontAwesomeIcon key={`half_${i}`} icon={faStarHalfAlt} className="text-yellow-500" />);
            } else {
                // Empty star
                stars.push(<FontAwesomeIcon key={`empty_${i}`} icon={regularStar} className="text-yellow-500" />);
            }
        }
        return stars;
    };
      ...
      <div className="flex justify-center items-center my-6">
        <div className="border p-3 rounded mx-2">
          <label htmlFor="category" className="mx-2">Category:</label>
          <select id="category" className="border px-2 py-1 rounded cursor-pointer">
        ...
        <div className="border p-3 rounded mx-2">
          <label htmlFor="sort" className="mx-2">Sort:</label>
          <select id="sort" className="border px-2 py-1 rounded cursor-pointer">
...
{/* Projects Section */}
<div className="w-full flex flex-wrap justify-center px-6">
                {projects.map((project) => (
                    <div key={project.id} className="border p-4 m-2 rounded-lg w-[calc(25%-1rem)] relative">
                        <img src={project.imageUrl} alt={project.title} className="w-full h-auto" />
                        <h3 className="text-lg font-bold text-[#bfa260]">{project.title}</h3>
